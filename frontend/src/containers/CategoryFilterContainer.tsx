...
const CategoryFilterContainer = ({ categories = [], selectedCategories = [], handleCategoryFilterChange } : CategoryFilterContainerProps ) => {
        ...
        <Box display="flex" flexDirection="row" flexWrap="wrap">
            {
                categories.length > 0 && (
                    categories.map((category: ICategory) => (
                        <FormControlLabel key={`${category._id}-filter`} control={<Checkbox
                            checked={!!selectedCategories.find(item => item._id === category._id) || false}
                            onChange={handleCategoryFilterChange}/>} name={category.name} label={category.name} sx={{ mr: 4 }} />
                    ))
                )
            }
        </Box>
