...
const TaskCardContent = ({
  ...
  const handleOnTaskTicked = () => {
    ...
    updateTask(task._id, {
      ...
      category: { categoryId: task.category?.categoryId || "" },
    });
  };
  ...
  return (
    <>
      <Stack direction="row" spacing={1}>
        {task.taskDate && displayDate && (
          <DateChip dateLabel={task.taskDate.toString()} />
        )}
      ...
      <Stack direction="row" spacing={0} justifyContent="flex-end">
        <IconButton onClick={handleOnEdit} color="primary" aria-label="Edit">
          <EditIcon />
        </IconButton>
        <IconButton
