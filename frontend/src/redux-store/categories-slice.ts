...
export const fetchCategories = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<ICategory[]>(API_CATEGORIES_ENDPOINT, {
...
export const updateCategory = createAsyncThunk(
  ...
  async (
    arg: { _id: string; name: string; colour?: string },
    { rejectWithValue },
  ) => {
    try {
...
export const deleteCategory = createAsyncThunk(
  ...
  async (arg: { _id: string }, { rejectWithValue }) => {
    try {
      await axios.delete<ICategory>(`${API_CATEGORIES_ENDPOINT}/${arg._id}`, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
      });
      return {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateCategory.fulfilled, (state, action) => {
      ...
      const index = state.categories.findIndex(
        (element: ICategory) => element._id === updatedCategory._id,
      );
    ...
    builder.addCase(deleteCategory.fulfilled, (state, action) => {
      state.categories = state.categories.filter(
        (c: ICategory) => c._id !== action.payload._id,
      );
    });
  },
});
