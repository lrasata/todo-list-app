...
export const fetchDueTodayTasks = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<ITask[]>(API_TASKS_DUE_TODAY, {
...
export const fetchOverdueTasks = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<ITask[]>(API_TASKS_OVERDUE, {
        withCredentials: true,
      });
      return {
        tasks: response.data,
...
export const deleteTask = createAsyncThunk(
  ...
  async (arg: { id: string }, { rejectWithValue }) => {
    try {
      await axios.delete(`${API_TASKS_ENDPOINT}/${arg.id}`, {
        withCredentials: true,
      });
      return {
        _id: arg.id,
...
export const createTask = createAsyncThunk(
  ...
  async (arg: { task: Partial<ITask> }, { rejectWithValue }) => {
    try {
      const response = await axios.post<ITask>(API_TASKS_ENDPOINT, arg.task, {
...
export const updateTask = createAsyncThunk(
  "tasks/updateTask",
  async (
    arg: { id: string; updatedTask: Partial<ITask> },
    { rejectWithValue },
  ) => {
    try {
...
const tasksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(createTask.fulfilled, (state, action) => {
      // @ts-ignore
      state.dueTodayTasks = dateIsToday(dayjs(action.payload.task.taskDate))
        ? [action.payload.task, ...state.dueTodayTasks]
        : [...state.dueTodayTasks];
      // @ts-ignore
      state.filteredTasks = [action.payload.task, ...state.filteredTasks];
      // @ts-ignore
      state.overdueTasks = dateIsInThePast(dayjs(action.payload.task.taskDate))
        ? [action.payload.task, ...state.overdueTasks]
        : [...state.overdueTasks];
    });
    builder.addCase(updateTask.fulfilled, (state, action) => {
      const updatedTask = action.payload.task;
      // @ts-ignore
      state.filteredTasks = findAndUpdateTask(updatedTask, [
        ...state.filteredTasks,
      ]);
      ...
      if (dateIsToday(dayjs(updatedTask.taskDate))) {
        ...
        const index = state.dueTodayTasks.findIndex(
          (element: ITask) => element._id === updatedTask._id,
        );
      ...
      if (dateIsInThePast(dayjs(action.payload.task.taskDate))) {
        ...
        // @ts-ignore
        state.dueTodayTasks = removeIfExistTask(
          updatedTask,
          state.dueTodayTasks,
        );

        const index = state.overdueTasks.findIndex(
          (element: ITask) => element._id === updatedTask._id,
        );
        if (index >= 0) {
    ...
    builder.addCase(deleteTask.fulfilled, (state, action) => {
      state.dueTodayTasks = state.dueTodayTasks.filter(
        (t: ITask) => t._id !== action.payload._id,
      );
